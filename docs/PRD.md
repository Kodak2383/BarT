...
everything to the left of them is pushed out of view until the user asks for it, either by
clicking BarT's icon or by pressing a shortcut.
...
| Licence | **GPL-3.0.** Inherited from Ice, kept by decision after the Ice code came out (BT-15) | R2-Q2, corrected |
...
window, so that nobody has to discover it by surprise.
...
BarT's job here is to explain the gesture and to show the result. Performing it is the user's
part, and §7 says why.
...
- **macOS 27 assertion engine.** Bartender 7 advertises "zero mouse interruptions", which is the
  same technique. It restores automatic arranging *without* any foreign code, and it is the point at
...
## 7. Why the drag engine was removed
